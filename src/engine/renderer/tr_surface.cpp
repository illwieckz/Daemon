...
void Tess_SurfaceIQM( srfIQModel_t *surf ) {
	...
	/* This must run after the bone computation code or rendering will be
	buggy, and must run before the other loops because it returns early
	and then save CPU time.

	This test is false when r_vboModels is disabled, or GL_ARB_buffer_storage
	is unsupported, or the model has too much bones for the hardware.
	...
	int *modelNumWeights = model->numWeights;
	shaderVertex_t *tessVertex = tess.verts + tess.numVertexes;
	...
	if ( model->num_joints > 0 && model->blendWeights && model->blendIndexes )
	{
		...
		if ( tess.skipTangentSpaces )
		{
			...
			for ( ; tessVertex < lastVertex; tessVertex++,
				...
				modelTexcoord += 2, modelNumWeights++ )
			{
				vec3_t position, tmp;

				VectorClear( position );

				byte *lastBlendIndex = modelBlendIndex + *modelNumWeights;
				...
				modelBlendIndex += 4 - *modelNumWeights;
				modelBlendWeight += 4 - *modelNumWeights;
